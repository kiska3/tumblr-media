...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'tumblr-media-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('tumblr-media: %(item_name)s')
        ]
        ...
        if item_type == 'media:':
            ...
            for x in split_items:
                wget_args.extend(['--warc-header', 'tumblr-media: ' + x])
        else:
            raise Exception('Unknown item')
            
            list_url = '' #Insert where the text file will come from
